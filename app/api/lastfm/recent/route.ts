...
import { isLastfmAvailable, fetchRecentTracks } from '@/lib/importers/lastfm';

/**
 * GET /api/lastfm/recent?user=username&page=1&limit=50
 * 
 * Fetch recent tracks from a Last.fm user profile.
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch from Last.fm
    const result = await fetchRecentTracks({ username, page, limit });
    ...
    console.error('[api/lastfm/recent] Error:', error);
