...
/**
 * GET /api/search/artists?q=query&limit=20&offset=0&provider=spotify|tidal
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (query.length === 0) {
      return NextResponse.json({ artists: [], total: 0, nextOffset: null });
    }

    const { provider } = resolveMusicProviderFromRequest(request);
    return NextResponse.json(await provider.searchArtists(query, limit, offset));
  } catch (error: unknown) {
